...
from module.Down import DWTEncodeBlock
from module.MLLA import MLLABlock
...
class LSA(nn.Module):
    ...
    def __init__(self, p=0.9, eps=1e-6):
        super().__init__()
        self.eps = eps
        self.p = p
        self.factor = 1
    ...
    def forward(self, x):
        ...
        mix_x = (x - mean.reshape(x.shape[0], x.shape[1], 1)) / std.reshape(x.shape[0], x.shape[1], 1)
        mix_x = mix_x * gamma.reshape(x.shape[0], x.shape[1], 1) + beta.reshape(x.shape[0], x.shape[1], 1)
...
class SS2D_Convnext(nn.Module):
    def __init__(
        ...
        self.dim = dim
        self.block = ConvNextv2Block(self.dim)
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        # 判断是否需要使用
        if B == 1:
            # 当 B=1 时，直接使用原始特征图进行加法，不应用
            y_orgin = y1 + y2 + y3 + y4
            y = y_orgin  # 直接使用原始特征图
        else:
            # 当 B>1 时，应用lsa进行特征图混合
            y1_mixed = lsa(y1)
            y2_mixed = lsa(y2)
            y3_mixed = lsa(y3)
            y4_mixed = lsa(y4)
            y = y1_mixed + y2_mixed + y3_mixed + y4_mixed  # 混合特征图的加法
...
class SS2D(nn.Module):
    ...
    def forwardv2(self, x: torch.Tensor, **kwargs):
        ...
        if with_dconv:
            # x1 = x
            # self.block = ConvNextv2Block(dim=x1.shape[1]).cuda()
            # self.add_module("block", self.block)
            # print('block')
            # x1 = self.block(x1)
            x = self.conv2d(x) # (b, d, h, w)
...
class VSSM(nn.Module):
    ...
    @staticmethod
    def _make_downsample_v3(dim=96, out_dim=192, norm_layer=nn.LayerNorm, channel_first=False, use_wavelet=True):
        layers = []
        if not channel_first:
            layers.append(Permute(0, 3, 1, 2))
        down_block = DWTEncodeBlock(
            in_channels=dim,
            out_channels=out_dim,
            dwt_channels=dim,
        )
        layers.append(down_block)
        if not channel_first:
